...
export class ArticleResolver {
  constructor(
    ...
    private readonly incrementArticleCounterUsecase: IncrementArticleCounterUsecase,
  ) {}
  ...
  @Query(() => AdjacentArticlesDTO, {
    description: '获取相邻文章（前一篇和后一篇，仅限已发布文章）',
  })
  ...
  @ResolveField(() => CategoryDTO, { nullable: true, description: '文章分类' })
  async category(@Parent() article: ArticleDTO): Promise<CategoryDTO | null> {
    if (!article.categoryId) {
      return null;
    }
    return this.categoryQueryService.getCategoryById(article.categoryId);
  }

  @ResolveField(() => [TagDTO], { description: '文章标签列表' })
  async tags(@Parent() article: ArticleDTO): Promise<TagDTO[]> {
    return this.tagQueryService.getTagsByArticle(article.id);
  }
  ...
  async incrementViewCount(@Args('id') id: string): Promise<ArticleDTO> {
    await this.incrementArticleCounterUsecase.incrementViewCount(id);
  ...
  async incrementLikeCount(@Args('id') id: string): Promise<ArticleDTO> {
    await this.incrementArticleCounterUsecase.incrementLikeCount(id);
    return this.articleQueryService.getArticleById(id) as Promise<ArticleDTO>;
  }
}
